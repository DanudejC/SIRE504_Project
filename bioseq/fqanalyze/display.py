...
def show_alldata(data,barcodeList,filterQ,filterL):
    ...
    for x in data:
        ...
        elif int(aList["Average_Qscore"]) >= filterQ  :
               if filterL == 0  :
                    avg_score_data.append(aList["Average_Qscore"])
                    length_data.append(aList["Read_length"])                    
    ...
    for x in barcodeList :
        ...
        get_barcodeinfo_result = get_barcodeinfo(bcdata,filterQ,filterL)
        sum_filter_total_reads += int(get_barcodeinfo_result[0])
        html2 +=  get_barcodeinfo_result[1]
        filter_status = get_barcodeinfo_result[2]
        
    
    if (filter_status == True) :
        total_reads = str(sum_filter_total_reads)
...
def get_barcodeinfo(data,filterQ,filterL):
    ...
    html += ' 		</tr>'
    result_return.append(html)
    result_return.append(filter_status)
